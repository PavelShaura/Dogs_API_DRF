...
class Breed(models.Model):
    ...
    friendliness = models.IntegerField(
        _("friendliness"), choices=[(i, i) for i in range(1, 6)]
    )
    trainability = models.IntegerField(
        _("trainability"), choices=[(i, i) for i in range(1, 6)]
    )
    shedding_amount = models.IntegerField(
        _("shedding amount"), choices=[(i, i) for i in range(1, 6)]
    )
    exercise_needs = models.IntegerField(
        _("exercise needs"), choices=[(i, i) for i in range(1, 6)]
    )
...
class Dog(models.Model):
    ...
    breed = models.ForeignKey(
        Breed, on_delete=models.CASCADE, related_name="dogs", verbose_name=_("breed")
    )
